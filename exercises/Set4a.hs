...
rangeOf xs = rangeOf' 9999 (-9999) xs

rangeOf' :: (Num a, Ord a) => a -> a -> [a] -> a
rangeOf' min max [] = max - min
rangeOf' min max (x:xs) 
  | x > max && x < min = rangeOf' x x xs
  | x > max            = rangeOf' min x xs
  | x < min            = rangeOf' x max xs
  | otherwise          = rangeOf' min max xs
...
longest :: (Ord a) => [[a]] -> [a]
longest xs = longest' xs [] (head $ head xs)

longest' :: (Ord a) => [[a]] -> [a] -> a -> [a]
longest' [] ys _        = ys
longest' (xs:xss) ys current = if length xs > length ys
                                  then longest' xss xs (head xs)
                                  else if length xs == length ys
                                          then if head xs > head ys
                                                  then longest' xss ys (head ys)
                                                  else longest' xss xs (head xs)
                                          else longest' xss ys current
...
winner scores player1 player2 = 
        case (Map.lookup player1 scores, Map.lookup player2 scores) of
          (Just x, Nothing) -> player1
          (Nothing, Just y) -> player2
...
-- freqs xs = freqsAlter' xs Map.empty

-- challenge 1 & 2
freqs xs = foldr (\c acc -> Map.alter fa c acc) Map.empty xs

freqs' :: (Eq a, Ord a) => [a] -> Map.Map a Int -> Map.Map a Int
freqs' [] map     = map
freqs' (x:xs) map = case Map.lookup x map of
                     Nothing -> freqs' xs $ Map.insert x 1 map
                     Just y  -> freqs' xs $ Map.insert x (y+1) map

fa :: (Num a) => Maybe a -> Maybe a
fa x = case x of 
         Nothing -> Just 1
         Just y -> Just (y+1)

freqsAlter' :: (Eq a, Ord a) => [a] -> Map.Map a Int -> Map.Map a Int
freqsAlter' [] map = map
freqsAlter' (x:xs) map = freqsAlter' xs $ Map.alter fa x map
...
transfer from to amount bank
  ...
  | otherwise                                        = credit to (debit from bank)
        where credit r bk = Map.adjust (\x -> x + amount) r bk
              debit r bk = Map.adjust(\x -> x - amount) r bk
...
maxIndex arr = fst $ head $ sortBy largest $ assocs arr
