...
isums' n accumulater = do
        if n /= 0
           then do
                   word <- getLine
                   let number = read word :: Int
                   putStrLn $ show (number + accumulater)
                   isums' (n-1) (number + accumulater)
           else return accumulater
