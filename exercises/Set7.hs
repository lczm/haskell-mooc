...
member e (Set [])     = False
member e (Set (x:xs)) = if e == x
                           then True
                           else member e (Set xs)

-- add a member to a set
add :: (Ord a) => a -> Set a -> Set a
add e (Set xs) = if member e (Set xs)
                    then Set xs
                    else Set (takeWhile (<e) xs ++ [e] ++ dropWhile (<=e) xs)
...
data State = Start | Add | FlourWithoutSugar | SugarWithoutFlour | Mixing | Baking | Finished | Error
  deriving (Eq,Show)

step :: State -> Event -> State
step Finished _                 = Finished -- Anything that is finished will stay finished
step Start AddEggs              = Add
step Add AddFlour               = FlourWithoutSugar
step Add AddSugar               = SugarWithoutFlour
step FlourWithoutSugar AddSugar = Mixing
step SugarWithoutFlour AddFlour = Mixing
step Mixing Mix                 = Baking
step Baking Bake                = Finished
step Start _                    = Error
step Add _                      = Error
step FlourWithoutSugar _        = Error
step SugarWithoutFlour _        = Error
step Mixing _                   = Error
step Baking _                   = Error
step Error _                    = Error -- an error thats an error
...
instance (Eq a, Ord a) => Monoid (Set a) where
  mempty = Set []
  -- or the empty set could be used
...
passwordAllowed s (MinimumLength i) = if length s < i
                                         then False
                                         else True
passwordAllowed s (ContainsSome []) = False
passwordAllowed s (ContainsSome (x:xs)) = if elem x s
                                             then True
                                             else passwordAllowed s (ContainsSome xs)
passwordAllowed s (DoesNotContain []) = True
passwordAllowed s (DoesNotContain (x:xs)) = if elem x s
                                               then False
                                               else passwordAllowed s (DoesNotContain xs)
passwordAllowed s (And pw1 pw2) = passwordAllowed s pw1 && passwordAllowed s pw2
passwordAllowed s (Or pw1 pw2) = passwordAllowed s pw1 || passwordAllowed s pw2
...
operation op x y = Operation op x y
