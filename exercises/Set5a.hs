...
up p = MkPosition { x = getX p, y = getY p + 1 }

-- right increases the x value of a position by one
right :: Position -> Position
right p = MkPosition { x = getX p + 1, y = getY p }
...
data UpDown = UpDownIncreasing Int | UpDownDecreasing Int
  deriving Show

-- zero is an increasing counter with value 0
zero :: UpDown
zero = UpDownIncreasing 0

-- get returns the counter value
get :: UpDown -> Int
get (UpDownIncreasing v) = v
get (UpDownDecreasing v) = v

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (UpDownIncreasing v) = UpDownIncreasing (v + 1)
tick (UpDownDecreasing v) = UpDownDecreasing (v - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownIncreasing v) = UpDownDecreasing v
toggle (UpDownDecreasing v) = UpDownIncreasing v
...
rgb (Invert x) = [abs $ head (rgb x) - 1,
                  abs $ rgb x !! 1 - 1,
                  abs $ rgb x !! 2 - 1]

rgb (Mix x y) = [(head (rgb x) + head (rgb y)) / 2,
                 ((rgb x !! 1) + (rgb y !! 1)) / 2,
                 ((rgb x !! 2) + (rgb y !! 2)) / 2]
...
fromBin :: Bin -> Int
fromBin b = fromBin' b 1

fromBin' :: Bin -> Int -> Int
fromBin' (O End) count = 0
fromBin' (I End) count = count
fromBin' (O b) count = 0 + fromBin' b count*2
fromBin' (I b) count = count + fromBin' b count*2

toBin :: Int -> Bin
toBin b = toBin' b (O End)

toBin' :: Int -> Bin -> Bin
toBin' count bin = if count == 0 
                      then bin
                      else toBin' (count-1) (inc bin)
