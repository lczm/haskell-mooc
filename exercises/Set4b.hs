...
largestHelper x acc = if length acc == 0 -- defualt case
                         then x:acc
                         else if head acc <= x -- if head is smaller / equal
                                 then if head acc == x -- if equal
                                         then x:acc
                                         else [x] -- if smaller
                                 else acc
...
lastHelper x Nothing = Just x -- This is the first call
lastHelper x acc = acc        -- Once this is called, it'll always give the first element
