...
-- weth count copies of start and ends with end.
--
...
buildList start count end = buildList' start count end False []

buildList' :: Int -> Int -> Int -> Bool -> [Int] -> [Int]
buildList' start count end finishEnd xs = if not finishEnd
                                             then buildList' start count end True (end:xs)
                                             else if count /= 0
                                                   then buildList' start (count-1) end True (start:xs)
                                                   else xs
...
mylast :: a -> [a] -> a
mylast def [] = def
mylast def [x] = x -- this case only hits when there is only one element in the list
...
indexDefault xs i def = if i < 0
                           then def
                           else indexDefault' xs i 0 def

indexDefault' :: [a] -> Int -> Int -> a -> a
indexDefault' [x] i count def = if count == 0 && i == 0
                                   then x
                                   else if i == count 
                                           then x
                                           else def
indexDefault' (x:xs) i count def = if count == i
                                      then x
                                      else indexDefault' xs i (count+1) def
...
sorted (x1:x2:xs) = if x1 <= x2
                       then sorted (x2:xs) -- bring the second var into the list again
                       else False
...
sumsOf :: [Int] -> [Int]
sumsOf []     = []
sumsOf xs = reverse' (sumsOf' xs 0 [])

sumsOf' :: [Int] -> Int -> [Int] -> [Int]
sumsOf' [x] current xs = x+current:xs
sumsOf' (x:xs) current arr = sumsOf' xs (current+x) (x+current:arr)

-- a little cheating but... :(
-- just a reverse function to make life a lot easier...
reverse' l = rev l []
  where
    rev [] a     = a
