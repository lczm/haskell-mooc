...
--                  then fst $ splitAt (length s `div` 2) s
--                  else fst $ splitAt ((length s `div` 2) + 1) s
...
while check update value = if prec 
                              then while check update (update value)
                              else value
                                      where prec = check value
...
multiCompose xs s = multiCompose' (reverse xs) s

multiCompose' :: [a -> a] -> a -> a
multiCompose' [] s = s
multiCompose' (x:xs) s = multiCompose' xs (x s)
...
interpreter' [] output x y = reverse output
interpreter' (c:cs) output x y = case c of
                             "up" -> interpreter' cs output x (y+1)
                             "down" -> interpreter' cs output x (y-1)
                             "left" -> interpreter' cs output (x-1) y
                             "right" -> interpreter' cs output (x+1) y
                             "printX" -> interpreter' cs (show x : output) x y
                             "printY" -> interpreter' cs (show y : output) x y
